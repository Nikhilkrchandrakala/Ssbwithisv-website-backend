...
.then(async () => {
  ...
  for (const student of students) {
    ...
    const piqMsg = `Candidate ${student.name} has completed their timed psychological test battery and uploaded their PIQ files.`;
    createNotif(student.assignedGTO, "PIQ & Dossier Ready for Review", piqMsg);
    createNotif(student.assignedIO, "PIQ & Dossier Ready for Review", piqMsg);
    if (isPsychPath) createNotif(student.assignedPsych, "PIQ & Dossier Ready for Review", piqMsg);
    if (isToPath) createNotif(student.assignedTO, "PIQ & Dossier Ready for Review", piqMsg);
    
    for (const admin of superAdmins) {
        createNotif(admin._id, "PIQ & Dossier Ready for Review", piqMsg);
    }
